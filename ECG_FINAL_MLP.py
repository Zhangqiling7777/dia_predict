...
#心电数据
file5 = pd.read_csv('train_ecg_final.csv')
...
#心电数据
testfile5 = pd.read_csv('test_ecg_final.csv')
...
train_contact = pd.merge(contact, file5_data, on='f.eid',how='left')  
...
testfile5_data = pd.DataFrame(testfile5)
test_merge1 = pd.merge(testfile1_data, testfile2_data, on='f.eid')
test_merge2 = pd.merge(testfile3_data, testfile4_data, on='f.eid')
test_contact1 = pd.merge(test_merge1, test_merge2, on='f.eid')
test_contact = pd.merge(test_contact1, testfile5_data, on='f.eid',how='left')
...
# 连续数据 离散数据 训练集
discrete_data = train_contact[discrete_columns]
continous_data = train_contact.drop(columns=discrete_data)
...
train_data = final_data.drop(columns=['f.eid','T2D'])
train_labels = final_data[['T2D']]
test_data = test_final.drop(columns=['f.eid','T2D'])
...
# #输出前200的特征
# top_200_features = feature_importance_df.head(20)
...
class MLP_Model(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes, num_hidden_layers=2, dropout_prob=0.5):
        ...
        for _ in range(num_hidden_layers):
            ...
            self.hidden_layers.append(nn.Dropout(dropout_prob))  # Add dropout for regularization
    ...
    def forward(self, x):
        ...
        # Apply batch normalization before final output
        x = self.batch_norm(x)
        x = self.fc_out(x)
...
input_size = train_data.shape[1]  # 输入特征的数量
...
train_data_tensor = torch.tensor(train_data.values).float()  # 3200, 100
